...
def training_epoch(model: LanguageModel, optimizer: torch.optim.Optimizer, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        # Обрезаем по максимальной длине в батче
        max_len = lengths.max().item()
        indices = indices[:, :max_len]

        optimizer.zero_grad()
        logits = model(indices, lengths)  # (B, max_len, vocab_size)

        # Сдвиг: предсказываем токен t+1 по токену t
        # logits[:, :-1, :] → предсказания для позиций 0..T-2
        # indices[:, 1:]    → таргеты (токены 1..T-1)
        loss = criterion(
        ...
        train_loss += loss.item() * lengths.shape[0]
...
def validation_epoch(model: LanguageModel, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        val_loss += loss.item() * lengths.shape[0]
